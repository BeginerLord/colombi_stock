...
const RegisterProduct: React.FC<RegisterProductProps> = ({
  ...
  const { createProductMutation, isPending: isPendingProduct } =
    useCreateProduct();

  const { isLoading: isLoadingSuppliers, suppliers } = UseGetAllSuppliers(
    0,
    10,
    "name",
    "asc"
  ...
  const createProductSuccess: SubmitHandler<ProductModeltDto> = async (
    data
  ) => {
    if (selectedProduct) {
  ...
  return (
    <div className={styles.container}>
      <h1 className={styles.title}>
        {selectedProduct ? "Actualizar Producto" : "Registro de Producto"}
      </h1>
      <form onSubmit={handleSubmit(createProductSuccess)}>
        ...
        <TextInput
          label="Stock Min"
          type="number"
          {...register("stockMin", { required: true })}
        />
        ...
        <ButtonComponent
          name={selectedProduct ? "Actualizar" : "Enviar"}
          type="submit"
          disabled={isPendingProduct || isUpdating}
        />
      </form>
    </div>
  );
